...
public class ChessMatch {
	...
	public boolean[][] possibleMoves(ChessPosition sourcePosition) {
		// convert position to position of a matrix
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == null) {
				...
				if (p.getColor() == Color.WHITE) {
					pPosition = new Position(target.getRow() + 1, target.getColumn());
				} else {
					pPosition = new Position(target.getRow() - 1, target.getColumn());
				}
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
				...
				Position pPosition;
				if (p.getColor() == Color.WHITE) {
					pPosition = new Position(3, target.getColumn());
				} else {
					pPosition = new Position(4, target.getColumn());
				}
				board.placePiece(pawn, pPosition);
			}
		}
	}
